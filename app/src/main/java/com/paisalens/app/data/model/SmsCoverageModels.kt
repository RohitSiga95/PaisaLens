...
fun areLikelyDuplicateSms(
    ...
    if (first.occurredAt == second.occurredAt) return true
    return hasStableSmsTransactionReference(first.rawMessage) &&
        hasStableSmsTransactionReference(second.rawMessage) &&
        kotlin.math.abs(first.occurredAt - second.occurredAt) <= windowMillis
}

/** Windowed auto-merge requires a bank reference; an exact timestamp also identifies one SMS row. */
fun hasStableSmsTransactionReference(body: String): Boolean =
    STABLE_TRANSACTION_REFERENCE_PATTERN.containsMatchIn(body)
...
private val STABLE_TRANSACTION_REFERENCE_PATTERN = Regex(
    "(?i)\\b(?:ref(?:erence)?|rrn|utr|upi\\s+ref(?:erence)?(?:\\s+no)?|txn(?:\\s+id)?|transaction\\s+id)" +
        "\\s*[:#.-]?\\s*[a-z0-9-]{6,}\\b",
)
